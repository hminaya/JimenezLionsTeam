...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
 ...
 <%
ArrayList<IEntidad> listaInventario = (ArrayList) request.getAttribute("lista.inventario");
if(listaInventario == null){
	//out.println("No hay Productos registrados en el Sistema");
}
else{

        double CantidadActual, CantidadMinima;
        String InventarioDecrip;
	for (IEntidad entidad : listaInventario)
	{
            String color = (listaInventario.indexOf(entidad) % 2 == 0)?"#CEF6F5":"#E0E0F8";
            EntidadInventario inventario = (EntidadInventario) entidad;
            InventarioDecrip=inventario.getNombreProducto();
            CantidadActual=inventario.getCantidadActual();
            CantidadMinima=inventario.getCantidadMinimaRequerida();
...
%>
		<tr class="odd gradeA">
			<td><% out.print( inventario.getNombreProducto()); %></td>
			<td><% out.print(CantidadActual ); %></td>
			<td><% out.print(CantidadMinima ); %></td>
...
%>
  ...
  <input type="submit" name="apiquisquellano" value="Conectar con APIQuisquellano">
			</div>
